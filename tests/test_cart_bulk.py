...
def test_add_bulk_creates_all_items(cart, product_factory):
    items = [
        {
            "product": product_factory(name="Bulk1"),
            "unit_price": Decimal("10.00"),
            "quantity": 1,
        },
        {
            "product": product_factory(name="Bulk2"),
            "unit_price": Decimal("20.00"),
            "quantity": 2,
        },
        {
            "product": product_factory(name="Bulk3"),
...
def test_add_bulk_returns_list_of_item_instances(cart, product_factory):
    result = cart.add_bulk(
        [
            {
                "product": product_factory(name="Return"),
                "unit_price": Decimal("10.00"),
                "quantity": 2,
            },
        ]
...
def test_add_bulk_raises_on_invalid_quantity_and_rolls_back_earlier_items(
    cart, product_factory
):
